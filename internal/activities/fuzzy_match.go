...
Given a user query and a list of known artefact names, return the single best match as JSON.
If nothing is a reasonable match, return an empty string for matched_id.
Respond with valid JSON only — no markdown, no extra text:
{"matched_id": "the-matching-name-or-empty-string"}`

// FuzzyMatch asks the LLM to resolve a free-form query to one of the known artefact names.
